...
export interface SlideToConfirmProps {
  ...
  width?: number;
  /* optional action to trigger on confirmation commit */
  onConfirm?: () => void;
  /* track label before confirmation */
  label?: string;
  /* badge label after confirmation */
  confirmedLabel?: string;
  /* color theme variant */
  variant?: 'default' | 'danger' | 'destructive' | 'warning' | 'success';
...
export function SlideToConfirm({
  ...
  const finish = () => {
    ...
    try {
      ...
      console.error("[SlideToConfirm Error on confirm callback]:", err);
    }
  ...
  return (
    <div
      className={`sld ${className}`}
      data-variant={variant}
      data-disabled={disabled || undefined}
      style={{ width: span, height: H }}
    >
      <motion.div
        className="sld-track"
        ref={track}
...
// Export alias matching the original name
export const SlideConfirm = SlideToConfirm;
